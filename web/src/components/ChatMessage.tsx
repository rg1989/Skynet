...
import type { Message, MessageMedia } from '../store';
...
function MediaAttachment({ media }: { media: MessageMedia }) {
  ...
  if (hasError) {
    return (
      <div className="mt-2 p-2 bg-red-900/20 border border-red-500/30 rounded-lg text-xs text-red-400">
    ...
    return (
      <div className="mt-2">
        <img
          ...
          className={`rounded-lg border border-slate-600/50 cursor-pointer transition-all ${
            isExpanded ? 'max-w-full' : 'max-w-[300px] max-h-[200px] object-cover'
          }`}
          onClick={() => setIsExpanded(!isExpanded)}
          onError={() => setHasError(true)}
        />
        {media.caption && (
          <div className="text-xs text-slate-500 mt-1">{media.caption}</div>
        )}
      </div>
    );
  }
    ...
    return (
      <div className="mt-2">
        <video
          controls
          src={media.url}
          className="max-w-full rounded-lg border border-slate-600/50"
          onError={() => setHasError(true)}
        />
        {media.caption && (
          <div className="text-xs text-slate-500 mt-1">{media.caption}</div>
  ...
  return (
    <div className="mt-2">
      <a
        ...
        className="inline-flex items-center gap-2 px-3 py-2 bg-slate-700/50 hover:bg-slate-700 rounded-lg text-sm text-slate-300 transition-colors"
      >
