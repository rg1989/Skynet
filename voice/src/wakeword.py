...
class WakeWordDetector:
    ...
    def __init__(
        ...
        self._tts_end_time: float = 0  # When cooldown started
        self._post_tts_cooldown_ms: float = 1000  # Cooldown period after TTS
        ...
        self._prediction_key: Optional[str] = None
    ...
    def process(self, audio_chunk: np.ndarray) -> WakeWordResult:
        ...
        if self._tts_end_time > 0:
            time_since_cooldown = (time.time() - self._tts_end_time) * 1000
            if time_since_cooldown < self._post_tts_cooldown_ms:
                return WakeWordResult(
                    state=self._state,
                    detected=False,
                    confidence=0.0,
        ...
        if self._prediction_key and self._prediction_key in predictions:
            ...
            for model_key in predictions.keys():
                ...
                if key_normalized == model_name_normalized or \
                   model_name_normalized in key_normalized or \
                   key_normalized in model_name_normalized:
        ...
        time_since_last = (current_time - self._last_detection_time) * 1000
        ...
        if confidence >= self.threshold and time_since_last >= self.debounce_ms:
            ...
            # Set detection lock for 2 seconds
            self._detection_lock_until = current_time + 2.0

            # Reset OWW model to clear accumulated audio
            if self._oww_model is not None:
    ...
    def is_ready(self) -> bool:
        """Check if wake word detector is ready."""
        if not self.enabled:
            return True
